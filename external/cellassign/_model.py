    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        #sourceFile = open('markerfile.txt', 'w')
        #sourceFile2 = open('adatavar.txt', 'w')
        #print(cell_type_markers, file = sourceFile)
        #print(adata.var_names, file = sourceFile2)
    ...
    def predict(self) -> pd.DataFrame:
        #no calculation, sudhu adata er part by part er jonno gamma value ber kre prediction matrix banano
        #parameter gula agei train hye ase, segula diye jst 1 bar gamma calculate
        """Predict soft cell type assignment probability for each cell."""
        adata = self._validate_anndata(None)
        scdl = self._make_data_loader(adata=adata) #adata k jst iterable kre
        predictions = []
        print("=============inside predict 1================")
        print("=============inside predict 2================")
        for tensors in scdl:
            #print("------------------------------In tensors loop-------------------------------")
            #print(tensors)
            ...
            #outputs = self.module.prediction_generative(**generative_inputs) #all 1
            outputs = self.module.generative_toggle(**generative_inputs) #all 1
            gamma = outputs["gamma"]
            #gamma tai amar prediction
            predictions += [gamma.cpu()] #jst gamma ta add kra
        print("------------------------------out tensors loop-------------------------------")
        return pd.DataFrame(
            torch.cat(predictions).numpy(), columns=self.cell_type_markers.columns #prediction matrix ta sajano
        )
        
    def hello_model():
        print("hello_model Tanvir")
    ...
    def train(
        ...
        train_size: float = 0.9, #total cell er eto percent train hbe
